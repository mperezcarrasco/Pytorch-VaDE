...
class TrainerVaDE:
    ...
    def compute_loss(self, x, x_hat, mu, log_var, z):
        ...
        h = (mu.unsqueeze(1) - self.VaDE.mu_prior.unsqueeze(0)).pow(2)
        h += log_var.exp().unsqueeze(1)
        h = torch.sum(self.VaDE.log_var_prior.unsqueeze(0) +\
            h / self.VaDE.log_var_prior.exp().unsqueeze(0), dim=2)
        log_p_z_given_c = 0.5 * torch.mean(torch.sum(gamma * h, dim=1))
        log_p_c = torch.mean(torch.sum(gamma * torch.log(p_c.unsqueeze(0) + 1e-9), dim=1))
    ...
    def compute_gamma(self, z, p_c):
        h = (z.unsqueeze(1) - self.VaDE.mu_prior.unsqueeze(0)).pow(2)
        h /= self.VaDE.log_var_prior.exp().unsqueeze(0)
        h += self.VaDE.log_var_prior.unsqueeze(0)
